...
namespace ksBroadcastingNetwork
{
    public class ACCUdpRemoteClient : IDisposable
    {
        ...
        public ACCUdpRemoteClient(string ip, int port, string displayName, string connectionPassword, string commandPassword, int msRealtimeUpdateInterval)
        {
            ...
            RaceEngineerPlugin.RaceEngineerPlugin.LogInfo("Requested broadcast connection");
            ...
            ShutdownAsnyc().ContinueWith(t =>
             {
                 if (t.Exception?.InnerExceptions?.Any() == true)
                     RaceEngineerPlugin.RaceEngineerPlugin.LogInfo($"Client shut down with {t.Exception.InnerExceptions.Count} errors");
                 else
                     RaceEngineerPlugin.RaceEngineerPlugin.LogInfo("Client shut down asynchronously");
        ...
        private async Task ConnectAndRun()
        {
            ...
            while (_client != null)
            {
                ...
                {
                    // Shutdown happened
                    RaceEngineerPlugin.RaceEngineerPlugin.LogInfo("Broadcast client shut down.");
                    break;
                }
                catch (Exception ex)
                {
                    // Other exceptions
                    RaceEngineerPlugin.RaceEngineerPlugin.LogInfo($"Couldn't connect to broadcast client. Err {ex}");
                }
            }
        }
        ...
        protected virtual void Dispose(bool disposing)
        {
            if (!disposedValue)
            {
                if (disposing)
                {
                    try
                    {
                        RaceEngineerPlugin.RaceEngineerPlugin.LogInfo("Disposed.");
                        if (_client != null)
                        {
                            _client.Close();
                            _client.Dispose();
